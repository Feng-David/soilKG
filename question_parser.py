...
class QuestionPaser:
    ...
    def parser_main(self, res_classify):
        ...
        for question_type in question_types:
            ...
            if question_type == 'disease_symptom':
                ...
                sql = self.sql_transfer(question_type, entity_dict.get('producer'))
    ...
    def sql_transfer(self, question_type, entities):
        ...
        # 查询疾病有哪些症状
            ...
            sql = ["MATCH (n:chemical) where n.name = '{0}' return n".format(i) for i in entities]
        ...
        # 查询疾病常用药品－药品别名记得扩充
            ...
            sql = ["MATCH (n:massif)-[r:massifTolandUse]->(m:landUse) where n.name = '{0}' return m.name".format(i) for i in entities]
            # sql2 = ["MATCH (m:Disease)-[r:recommand_drug]->(n:Drug) where m.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
            # sql = sql1 + sql2
        elif question_type == 'drug_qwds':
            sql = ["MATCH (n:massif)-[r:massifTochemical]->(m:chemical) where n.name = '{0}' return m.name".format(i) for
                   i in entities]
        elif question_type == 'prevent_qwds':
            sql = ["MATCH (n:massif)-[r:massifTovadoseZone]->(m:vadoseZone) where n.name = '{0}' return m.name".format(i) for
                   i in entities]
        elif question_type == 'cureway_qwds':
            sql = ["MATCH (n:massif)-[r:massifTochemical]->(m:chemical) where n.name = '{0}' return m.name".format(i) for
                   i in entities]
        ...
        # 已知药品查询能够治疗的疾病
            ...
            sql = ["MATCH (n:cityInfo)-[r:cityTocounty]->(m) where n.name = '{0}' return m.name".format(i) for i in entities]
            # sql2 = ["MATCH (m:Disease)-[r:recommand_drug]->(n:Drug) where n.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
            # sql = sql1 + sql2
        # 查询疾病应该进行的检查
        elif question_type == 'disease_check':
            sql = ["MATCH (n:massif)-[r:massifToindustry]->(m:industry) where n.name = '{0}' return m.name".format(i) for i in entities]

        # 查询荔湾区的相关地块
        elif question_type == 'check_disease':
            sql = ["MATCH (n:countyInfo)<-[r:massifTocounty]-(m:massif) where n.name = '{0}' return m.name".format(i) for i in entities]
